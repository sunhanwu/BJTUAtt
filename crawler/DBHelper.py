...
class DBHelper:
    ...
    def save_user(self, user):
        ...
        sql = "select id from fb_users where uuname = %s"
    ...
    def find_user_to_crawl(self):
        ...
        sql = "select fb_users.id, fb_users.uname, fb_users.uuname, fb_users.prof, fb_users.url  from fb_users where id = (select id from crawl_status where dbstatus = 1 order by id asc limit 1)"
        #self.logger.info(sql)
        cursor.execute(sql)
